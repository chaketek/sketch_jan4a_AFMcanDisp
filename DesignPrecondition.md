...
| CANトランシーバ | M5Stack CAN Unit (U085) | CA-IS3050G搭載 絶縁型CANトランシーバ          |

> **Note**: U085はCANトランシーバ（物理層）のみを提供。CANコントローラ（プロトコル処理）はESP32内蔵のTWAIを使用。

### 2.2 CAN通信アーキテクチャ

```
[CANバス H/L] <---> [CA-IS3050G] <---> [ESP32 TWAI] <---> [アプリケーション]
                    (トランシーバ)      (コントローラ)
                      U085内蔵         Core2内蔵
```
...
| レイヤー           | 担当デバイス         | 機能                               |
| ------------------ | -------------------- | ---------------------------------- |
| 物理層             | CA-IS3050G (U085)    | 差動信号の送受信、絶縁、保護       |
| データリンク層     | ESP32 TWAI           | フレーム生成/解析、エラー処理、ACK |
| アプリケーション層 | ESP32 (ソフトウェア) | シグナルデコード、UI表示           |
...
- ~~**ESP32-Arduino-CAN**~~: https://github.com/miwagner/ESP32-Arduino-CAN （※ESP32 Arduino Core 3.x非対応のため使用不可）
...
| 2026/01/05 | 1.4        | ハードウェア記述修正: U085はCA-IS3050G搭載トランシーバ（MCP2515ではない）、CANアーキテクチャ図追加 | -    |
